...
```mermaid
flowchart TD
    A{Is the value shared<br/>across projects?} -->|No| B{Does it change<br/>per environment?}
    A -->|Yes| G[Global variables]
    B -->|Yes, scaling/config| C[YAML variables]
    B -->|Yes, a credential| D[Secrets]
    B -->|No, one deployment only| E[Environment variables]
```

| You want to… | Use |
|---|---|
| Share config across multiple projects | **Global variables** |
| Scale one project per environment (`CPU`, `MEMORY`, `REPLICAS`) | [YAML variables](./yaml-variables.md) |
| Store a credential for a single project | [Secrets](./secrets-management.md) |
