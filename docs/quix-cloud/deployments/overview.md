...
| Deployment settings | Here you can select whether you want the application to run as a job or a service. You also choose the CPU and memory for each replica, either by picking a [deployment size](./deployment-sizes.md) from your organization's catalog or, unless your organization enforces size limits, by setting the values directly. The same panel sets the CPU and memory [reservation](./deployment-sizes.md#limits-and-requests) where your organization allows it. You can also provide some horizontal scaling by having more than one replica. A replica is an instance of the application running. When replicas are part of a consumer group, they can spread processing of streams across all replicas. |
